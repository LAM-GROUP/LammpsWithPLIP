...
/* ---------------------------------------------------------------------- */
double f_cut8(double rcut, double r)
{       
        double pi=3.1415926535897932;
        return 0.5*(1.0+cos(pi*(r/rcut)));
}
double f_cut8D(double rcut, double r)
{       
        double pi=3.1415926535897932;
        return -pi/rcut*0.5*sin(pi*(r/rcut));
}

double powbis8(double tmp,int l)
{
	double out=1;
...
double Lognormal(double A,double B,double r)
{       
	if (r-A<=0)
                return 0;
        else
...
double Lognormal_D(double A,double B,double r)
{       
	if (r-A<=0)
                return 0;
        else
...
PairLARS_3B_f8::PairLARS_3B_f8(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairLARS_3B_f8::compute(int eflag, int vflag)
{
...
void PairLARS_3B_f8::settings(int narg, char **arg)
{
...
void PairLARS_3B_f8::coeff(int narg, char **arg)
{
  int i,j,n;
...
void PairLARS_3B_f8::init_style()
{
...
double PairLARS_3B_f8::init_one(int i, int j)
{
...
void PairLARS_3B_f8::read_file(char *file)
{
...
void PairLARS_3B_f8::setup_params()
{
  int i,j,k,m,n;
  double rtmp;
...
void PairLARS_3B_f8::threebody(Param *paramijk,
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
	...
	for (int i_selected=0;i_selected<paramijk->N_selected;i_selected++)
	{
		if (paramijk->epsilon_all[i_selected] !=0)		// Useless as we already know N_selected
		{
			...
			{
				pow2=powbis8(cosinus,paramijk->powerl_all[i_selected]-1);
				pow1=pow2*cosinus;
			}
		
			double f1=Lognormal(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1 ) ;
			double f2=Lognormal(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2 ) ;

			double fcut1=f_cut8(paramijk->cut,r1);
			double fcut2=f_cut8(paramijk->cut,r2);
			...
			fprime=-inv1*(Lognormal_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1)*fcut1+f1*f_cut8D(paramijk->cut,r1))*fr2*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
			...
			fprime=-inv2*( Lognormal_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2) *fcut2+f2*f_cut8D(paramijk->cut,r2))*fr1*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
