...
PairLars_NB_xPot::PairLars_NB_xPot(LAMMPS *lmp) : PairEAM_POWER(lmp)
{
  one_coeff = 1;
  manybody_flag = 1;
}
...
/* ----------------------------------------------------------------------
   ...
   read EAM_POWER Finnis-Sinclair file
------------------------------------------------------------------------- */

void PairLars_NB_xPot::coeff(int narg, char **arg)
{
  int i,j;
  ...
  for (i = 3; i < narg; i++) {
    ...
    else error->all(FLERR,"No matching element in EAM_POWER potential file");
  }
...
void PairLars_NB_xPot::read_file(char *filename)
{
  Fs *file = fs;
  ...
  if (nwords != file->nelements + 1)
    error->all(FLERR,"Incorrect element names in EAM_POWER potential file");
  ...
  if (me == 0) {
    fgets(line,MAXLINE,fptr);
    nwords = sscanf(line,"%d %lg %d %lg %lg %d %lg",
           &file->nrho,&file->drho,&file->nr,&file->dr,&file->cut,&file->powerm,&file->coeffLars);
  }
  ...
  if ((nwords != 7) || (file->nrho <= 0) || (file->nr <= 0) || (file->dr <= 0.0))
...
void PairLars_NB_xPot::file2array()
{
  int i,j,m,n;
  ...
  // if atom type doesn't point to element (non-EAM_POWER atom in pair hybrid)
