...
/* ---------------------------------------------------------------------- */
double f_cut5(double rcut, double r)
{       
        double pi=3.1415926535897932;
        return 0.5*(1.0+cos(pi*(r/rcut)));
}
double f_cut5D(double rcut, double r)
{       
        double pi=3.1415926535897932;
        return -pi/rcut*0.5*sin(pi*(r/rcut));
}

double powbis5(double tmp,int l)
{
	double out=1;
...
PairLARS_3B_f5::PairLARS_3B_f5(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairLARS_3B_f5::compute(int eflag, int vflag)
{
...
void PairLARS_3B_f5::settings(int narg, char **arg)
{
...
void PairLARS_3B_f5::coeff(int narg, char **arg)
{
  int i,j,n;
...
void PairLARS_3B_f5::init_style()
{
...
double PairLARS_3B_f5::init_one(int i, int j)
{
...
void PairLARS_3B_f5::read_file(char *file)
{
  ...
  while (1) {
    ...
    for (int i=0;i<params[nparams].N_selected;i++)
    {
    	...
    	cout<<nparams<<"  "<<params[nparams].epsilon_all[i]<<endl;
    }
    nparams++;
  }
...
void PairLARS_3B_f5::setup_params()
{
  int i,j,k,m,n;
  double rtmp;
...
void PairLARS_3B_f5::threebody(Param *paramijk,
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
	...
	for (int i_selected=0;i_selected<paramijk->N_selected;i_selected++)
	{
		if (paramijk->epsilon_all[i_selected] !=0)		// Useless as we already know N_selected
		{
			...
			{
				pow2=powbis5(cosinus,paramijk->powerl_all[i_selected]-1);
				pow1=pow2*cosinus;
			}
		
			double f1=POW(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1 ) ;
			double f2=POW(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2 ) ;

			double fcut1=f_cut5(paramijk->cut,r1);
			double fcut2=f_cut5(paramijk->cut,r2);
			...
			fprime=-inv1*(POW_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1)*fcut1+f1*f_cut5D(paramijk->cut,r1))*fr2*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
			...
			fprime=-inv2*( POW_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2) *fcut2+f2*f_cut5D(paramijk->cut,r2))*fr1*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
