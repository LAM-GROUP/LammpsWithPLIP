...
double fcut1(double rcut, double r)
{
...
double fcut1D(double rcut, double r)
{
	double pi=3.1415926535897932;
...
double powbis1(double tmp,int l)
{
	double out=1;
...
PairLARS_3B_f1::PairLARS_3B_f1(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairLARS_3B_f1::compute(int eflag, int vflag)
{
...
void PairLARS_3B_f1::settings(int narg, char **arg)
{
...
void PairLARS_3B_f1::coeff(int narg, char **arg)
{
  int i,j,n;
...
void PairLARS_3B_f1::init_style()
{
...
double PairLARS_3B_f1::init_one(int i, int j)
{
...
void PairLARS_3B_f1::read_file(char *file)
{
...
void PairLARS_3B_f1::setup_params()
{
  int i,j,k,m,n;
  double rtmp;
...
void PairLARS_3B_f1::threebody(Param *paramijk,
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
	...
	for (int i_selected=0;i_selected<paramijk->N_selected;i_selected++)
	{
		if (paramijk->epsilon_all[i_selected] !=0)		// Useless as we already know N_selected
		{
			...
			{
				pow2=powbis1(cosinus,paramijk->powerl_all[i_selected]-1);
				pow1=pow2*cosinus;
			}
		
			double f1=Gaus(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1 ) ;
			double f2=Gaus(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2 ) ;

			double fcut11=fcut1(paramijk->cut,r1);
			double fcut12=fcut1(paramijk->cut,r2);

			double inv1=1.0/r1;
			double inv2=1.0/r2;


			double fr1=fcut11*f1;
			double fr2=fcut12*f2;
		
			fprime=-inv1*(Gaus_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1)*fcut11+f1*fcut1D(paramijk->cut,r1))*fr2*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
			...
			fprime=-inv2*(Gaus_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2) *fcut12+f2*fcut1D(paramijk->cut,r2))*fr1*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
