...
/* ---------------------------------------------------------------------- */
double f_cut4(double rcut, double r)
{       
        double pi=3.1415926535897932;
        return 0.5*(1.0+cos(pi*(r/rcut)));
}
double f_cut4D(double rcut, double r)
{       
        double pi=3.1415926535897932;
        return -pi/rcut*0.5*sin(pi*(r/rcut));
}

double powbis4(double tmp,int l)
{
	double out=1;
...
{
        if (A!=0)
                return pow(r,A)*exp(-B*r)/(pow(2,A)*exp(-B*2));
        else
                return exp(-B*r)/exp(-B*2);
...
{
        if (A!=0)
                return (A-B*r)*pow(r,A-1)*exp(-B*r)/(pow(2,A)*exp(-B*2));
        else
                return -B*exp(-B*r)/exp(-B*2);
}
...
PairLARS_3B_f4::PairLARS_3B_f4(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairLARS_3B_f4::compute(int eflag, int vflag)
{
...
void PairLARS_3B_f4::settings(int narg, char **arg)
{
...
void PairLARS_3B_f4::coeff(int narg, char **arg)
{
  int i,j,n;
...
void PairLARS_3B_f4::init_style()
{
...
double PairLARS_3B_f4::init_one(int i, int j)
{
...
void PairLARS_3B_f4::read_file(char *file)
{
...
void PairLARS_3B_f4::setup_params()
{
  int i,j,k,m,n;
  double rtmp;
...
void PairLARS_3B_f4::threebody(Param *paramijk,
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
	...
	for (int i_selected=0;i_selected<paramijk->N_selected;i_selected++)
	{
		if (paramijk->epsilon_all[i_selected] !=0)		// Useless as we already know N_selected
		{
			...
			{
				pow2=powbis4(cosinus,paramijk->powerl_all[i_selected]-1);
				pow1=pow2*cosinus;
			}
		
			double f1=STO(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1 ) ;
			double f2=STO(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2 ) ;

			double fcut1=f_cut4(paramijk->cut,r1);
			double fcut2=f_cut4(paramijk->cut,r2);
			...
			fprime=-inv1*(STO_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r1)*fcut1+f1*f_cut4D(paramijk->cut,r1))*fr2*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
			...
			fprime=-inv2*( STO_D(paramijk->p_all[i_selected],paramijk->q_all[i_selected],r2) *fcut2+f2*f_cut4D(paramijk->cut,r2))*fr1*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
