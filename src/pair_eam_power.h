...
PairStyle(eam_power,PairEAM_POWER)

#else

#ifndef LMP_PAIR_EAM_POWER_H
#define LMP_PAIR_EAM_POWER_H
...
class PairEAM_POWER : public Pair {
 public:
  ...
  int nrho,nr;
  int nfrho,nrhor,nz2r,powerm;
  double **frho,**rhor,**z2r,coeffLars;
  ...
  PairEAM_POWER(class LAMMPS *);
  virtual ~PairEAM_POWER();
  ...
  double memory_usage();
  void swap_eam_power(double *, double **);
 ...
 protected:
  ...
  struct Fs {
    char **elements;
    int nelements,nrho,nr,powerm;
    double drho,dr,cut,coeffLars;
    double *mass;
    double **frho,***rhor,***z2r;
  };
  Fs *fs;
