...
/* ---------------------------------------------------------------------- */
double f_cut7(double rcut, double r)
{       
...
double f_cut7D(double rcut, double r)
{       
...
double powbis7(double tmp,int l)
{
	double out=1;
...
double Lorentz(double A,int B,double r)
{       
	return 1/(powbis7((r-A),B)+1);
}
double Lorentz_D(double A,int B,double r)
{       
	return	-1/((powbis7((r-A),B)+1)*(powbis7((r-A),B)+1))*B*powbis7((r-A),B-1);
}
...
PairLARS_3B_f7::PairLARS_3B_f7(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairLARS_3B_f7::compute(int eflag, int vflag)
{
...
void PairLARS_3B_f7::settings(int narg, char **arg)
{
...
void PairLARS_3B_f7::coeff(int narg, char **arg)
{
  int i,j,n;
...
void PairLARS_3B_f7::init_style()
{
...
double PairLARS_3B_f7::init_one(int i, int j)
{
...
void PairLARS_3B_f7::read_file(char *file)
{
...
void PairLARS_3B_f7::setup_params()
{
  int i,j,k,m,n;
  double rtmp;
...
void PairLARS_3B_f7::threebody(Param *paramijk,
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
	...
	for (int i_selected=0;i_selected<paramijk->N_selected;i_selected++)
	{
		if (paramijk->epsilon_all[i_selected] !=0)		// Useless as we already know N_selected
		{
			...
			{
				pow2=powbis7(cosinus,paramijk->powerl_all[i_selected]-1);
				pow1=pow2*cosinus;
			}
		
			int B = static_cast<int>(paramijk->q_all[i_selected]);
			double f1=Lorentz(paramijk->p_all[i_selected],B,r1 ) ;
			double f2=Lorentz(paramijk->p_all[i_selected],B,r2 ) ;

			double fcut1=f_cut7(paramijk->cut,r1);
			double fcut2=f_cut7(paramijk->cut,r2);
			...
			fprime=-inv1*(Lorentz_D(paramijk->p_all[i_selected],B,r1)*fcut1+f1*f_cut7D(paramijk->cut,r1))*fr2*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
			...
			fprime=-inv2*( Lorentz_D(paramijk->p_all[i_selected],B,r2) *fcut2+f2*f_cut7D(paramijk->cut,r2))*fr1*pow1;
			if (paramijk->powerl_all[i_selected]!=0)
			{
