...
PairEAM_POWER::PairEAM_POWER(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  manybody_flag = 1;
...
void PairEAM_POWER::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    p -= m;
    p = MIN(p,1.0);
    fp[i] = powbis(rho[i],powerm-1)*powerm; //(coeff[0]*p + coeff[1])*p + coeff[2];
    if (eflag) {
      phi = coeffLars*powbis(rho[i],powerm); 
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	double rcut=6.0;
        ...
        recip = 1.0/r;
        psip = (fp[i]*rhojp + fp[j]*rhoip)*coeffLars; 
...
void PairEAM_POWER::settings(int narg, char **arg)
{
...
void PairEAM_POWER::coeff(int narg, char **arg)
{
...
double PairEAM_POWER::init_one(int i, int j)
{
...
void PairEAM_POWER::read_file(char *filename)
{
  ...
  if (me == 0) {
    ...
    if (fptr == NULL) {
      char str[128];
      sprintf(str,"Cannot open EAM_POWER potential file %s",filename);
      error->one(FLERR,str);
    }
  }
  ...
  if ((nwords != 5) || (file->nrho <= 0) || (file->nr <= 0) || (file->dr <= 0.0))
    error->all(FLERR,"Invalid EAM_POWER potential file");
...
void PairEAM_POWER::file2array()
{
  ...
  // add extra frho of zeroes for non-EAM_POWER types to point to (pair hybrid)
  // this is necessary b/c fp is still computed for non-EAM_POWER atoms

  for (m = 1; m <= nrho; m++) frho[nfrho-1][m] = 0.0;

  // type2frho[i] = which frho array (0 to nfrho-1) each atom type maps to
  // if atom type doesn't point to file (non-EAM_POWER atom in pair hybrid)
  ...
  // OK if map = -1 (non-EAM_POWER atom in pair hybrid) b/c type2rhor not used
  ...
  // if map = -1 (non-EAM_POWER atom in pair hybrid):
...
void PairEAM_POWER::array2spline()
{
  rdr = 1.0/dr;
  rdrho = 1.0/drho;
...
void PairEAM_POWER::interpolate(int n, double delta, double *f, double **spline)
{
...
void PairEAM_POWER::grab(FILE *fptr, int n, double *list)
{
  char *ptr;
  char line[MAXLINE];
...
double PairEAM_POWER::single(int i, int j, int itype, int jtype,
...
int PairEAM_POWER::pack_forward_comm(int n, int *list, double *buf,
...
void PairEAM_POWER::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairEAM_POWER::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairEAM_POWER::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double PairEAM_POWER::memory_usage()
{
...
void PairEAM_POWER::swap_eam_power(double *fp_caller, double **fp_caller_hold)
{
  double *tmp = fp;
  fp = fp_caller;
  *fp_caller_hold = tmp;
...
void *PairEAM_POWER::extract(const char *str, int &dim)
{
  dim = 2;
